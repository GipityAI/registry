...
export default async function agentWrite(ctx, { db, guid }) {
  const { api_key: apiKey, action, object: objectName, id, values } = ctx.body || {};
  try {
    if (!apiKey) {
    ...
    const result = await performWrite({ db, guid, object, actor, action, id, values });
    await db.query('UPDATE kit_api_keys SET last_used_at = NOW() WHERE id = $1', [key.id]);
    return result;
  } catch (err) {
