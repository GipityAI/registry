...
export default async function recordWrite(ctx, { db, guid }) {
  const { action, object: objectName, id, values, expect_updated_at: expectUpdatedAt, source } = ctx.body || {};
  try {
    ...
    return await performWrite({ db, guid, object, actor, action, id, values, expectUpdatedAt });
  } catch (err) {
