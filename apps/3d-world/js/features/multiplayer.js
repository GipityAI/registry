...
export function create(config, deps) {
  ...
  let localSid = null;
  // sid -> { F, r } of a just-received rocket hit, waiting to be kicked into
  // that peer's hit springs once its mesh exists. Consumed in update().
  ...
  return {
    ...
    async init() {
      ...
      if (room) {
        localSid = network.rt?.getSessionId?.() || null;
        if (localSid) player.setPlayerColor(colorForPeer(localSid));

        // Rocket hits: the victim's client broadcasts a 'hit' on the shared
        // 'rocket' channel; here we flinch the matching peer avatar. The
        // victim already flinched locally, so its own echo is ignored.
        const rocketCh = network.channel('rocket', { sync: 'messages' });
        rocketCh.on('hit', (data) => {
          if (!data || data.sid == null || data.sid === localSid) return;
          // {fx,fy,fz} force + {ox,oy,oz} impact offset, in the peer's local
          // frame. Held until update() can kick that peer's hit springs.
          pendingHits.set(data.sid, {
            F: { x: data.fx || 0, y: data.fy || 0, z: data.fz || 0 },
            r: { x: data.ox || 0, y: data.oy || 0, z: data.oz || 0 },
          });
        });
      }
    ...
    update(dt) {
      ...
      for (const [sid, peer] of network.avatars.peers()) {
        ...
        // Apply any just-received rocket hit now that the mesh exists - kick
        // its hit springs, which then settle on their own.
        const ph = pendingHits.get(sid);
        if (ph) {
          player.kickHitSprings(mesh, ph.F, ph.r);
          pendingHits.delete(sid);
        }
