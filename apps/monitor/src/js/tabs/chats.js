...
export async function renderChatsTab(api, { range, appGuid }) {
  ...
  if (!res.data.length) {
    tbody.innerHTML = emptyState(5, {
      icon: '💬',
      message: 'Recorded coding-agent sessions appear here - titles, tokens, and full transcripts. Recording is off until you turn it on for a project.',
      tryit: 'gipity init --capture',
    });
    return;
  }
