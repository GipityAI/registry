/**
 * Data tab: three sub-tabs covering persistent storage:
 *   - Files: total bytes (live count) + per-project breakdown + storage cost chart.
 *   - Browse: one project's files, their content and version history (browse.js).
...
import { renderBrowseSubtab } from './browse.js';
...
const tabs = subTabs('data', 'data', ['files', 'browse', 'db']);
...
export async function renderDataTab(api, filters) {
  ...
  switch (currentSub) {
    ...
    case 'browse':  return renderBrowseSubtab(api, filters);
