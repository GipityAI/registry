...
export async function renderOverviewTab(api, { range, appGuid, projectId }) {
  ...
  // Only real failures: completed, running, paused and queued runs are fine.
  const isFailure = (r) => ['failed', 'error', 'timeout'].includes((r.status || '').toLowerCase());
  const failedWf = (workflows.data.recent || []).find(isFailure);
  ...
  const failedJob = (jobs.data.recent || []).find(isFailure);
